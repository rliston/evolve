...
def radius(pop):
    ...
    #return max(1.414,args.gamma*np.log(pop+1))
    #return max(1.414,args.gamma*np.log(pop+1)*np.log(pop+1))
    #return max(args.minrad,np.sqrt(pop/np.pi))
    #return max(args.minrad,np.sqrt(pop))

def log(hdr,n,l,lmax,m,pop,r,k,backtrack,gen):
    print('{:10} wall {} n {:6d} LIFE {:6d} lmax {:6d} pop {:6d} m {:6d} r {:12.8f} k {:6d} backtrack {:6.0f} gen {:6.0f}'.format(hdr,datetime.datetime.now(),n,l,lmax,pop,m,r,k,backtrack,gen))
...
#            xy.extend([(int(x0+random.normalvariate(0,r)),int(y0+random.normalvariate(0,r))) for i in range(m)])
        ...
        if ent[-1]<0:
            log('RUNAWAY',n,l,lmax,len(xy),pat.population,r,k,backtrack,len(best))
            bb = pat.bounding_box
...
n=0 # total count
k=0 # backtrack count
backtrack=0 # backtrack threshold
best=[]
#side = args.side
#space = args.space
...
ipat = np.random.normal(radius(args.ipop),radius(args.ipop),size=[args.ipop,2])
...
### ibackground = np.random.uniform(-3*radius(args.ipop),3*radius(args.ipop),size=[args.ipop,2])
### pat[ibackground] ^= 1
...
while True:
    #backtrack = args.backtrack
    #backtrack = np.log(n+1)
    #backtrack = np.log(len(best)+1)
    #backtrack = np.sqrt(len(best))
    #backtrack = len(best)
#    if lmax<5000:
#        side=0
#    elif lmax<15000:
...
#        side=3
    ...
    if l > lmax:
        ...
        #lmax += args.period
        lmax = l
        backtrack = lmax/args.batch
        #l = lifespan(pat,0)
        best.append([pat.coords(),l])
        bb = pat.bounding_box
        if l>ath:
            ath=l
            log('BEST',n,l,lmax,len(xy),pat.population,r,k,backtrack,len(best))
            log('ATH',n,l,lmax,len(xy),pat.population,r,k,backtrack,len(best))
...
#        log('PATIENCE',n,l,lmax,len(xy),pat.population,r,k)
    ...
    if k > backtrack:
        ...
        for j in range(b):
            best.pop()
        if len(best)==0:
            log('EMPTY',n,l,lmax,0,pat.population,r,k,backtrack,len(best))
            break
        #(xy,l) = best.pop()
        #best.pop()
        (xy,l) = best[-1]
...
#            break
        log('BACKTRACK',n,l,lmax,0,pat.population,r,k,backtrack,len(best))
        k=0
        if n>args.watchdog*wd:
            log('WATCHDOG',n,l,lmax,0,pat.population,r,k,backtrack,len(best))
            break
