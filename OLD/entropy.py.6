...
parser.add_argument('--timeout', help='timeout in units of n', default=20000, type=int)
...
def log(hdr,n,e,emax,m,pop,r,l,etol):
    print('{:10} wall {} n {:6d} LIFE {:12.4f} emax {:12.4f} pop {:6d} m {:6d} r {:12.8f} l {:6d} etol {:12.10f}'.format(hdr,datetime.datetime.now(),n,e,emax,pop,m,r,l,etol))
...
#    etol = emax*args.tol
    ...
    if e>etol:
        ...
        #t0=time.time()
        n0=n
        log('BEST',n,e,emax,len(xy),pat.population,r,e,(1. - (1./r)))
        bb = pat.bounding_box
        if fn is not None and args.summary:
            os.remove(fn)
        fn = '{}/best_E{:08.0f}_L{:06d}_seed{:09d}_n{:09d}.rle'.format(args.results,e,int(e),args.seed,n)
        #pat.write_rle('{}/best_E{:08.0f}_L{:06d}_seed{:09d}_n{:09d}.rle'.format(args.results,e,int(e),args.seed,n), header='#CXRLE Pos={},{}\n'.format(bb[0],bb[1]), footer=None, comments=str(args), file_format='rle', save_comments=True)
